...
levelup(dir, {}, (err, db) => {
    ...
    r.defineCommand('get', {
        help: 'obtains the value of the given key from the database',
        action: function (key) {
    ...
    r.defineCommand('put', {
        help: 'sets the value of the given key in the database',
        action: function (keyAndVal) {
            let key, val;
            [key, val] = keyAndVal.split(/ +/);
            this.lineParser.reset();
            this.bufferedCommand = '';
            db.put(key, val, (err, data) => {
                if (err) {
                    console.error(err);
                } else {
                    console.log(data);
    ...
    r.defineCommand('del', {
        help: 'removes a key from the leveldb database',
        action: function (key) {
            this.lineParser.reset();
            this.bufferedCommand = '';
            db.del(key, (err) => {
    ...
    r.defineCommand('stats', {
        help: 'obtains the leveldb stats from the underlying database',
        action: function () {
            this.lineParser.reset();
            this.bufferedCommand = '';
    ...
    r.defineCommand('sstables', {
        help: 'obtains the leveldb sstables',
        action: function () {
            this.lineParser.reset();
            this.bufferedCommand = '';
    ...
    r.defineCommand('count', {
        help: 'show the number of records in the database',
        action: function () {
            this.lineParser.reset();
            this.bufferedCommand = '';
    ...
    r.defineCommand('size', {
        help: 'show the total size in bytes of the database',
        action: function () {
            this.lineParser.reset();
            this.bufferedCommand = '';
            var stream = db.createReadStream();
    ...
    r.defineCommand('keys', {
        help: 'show the list of keys in the database',
        action: function () {
            this.lineParser.reset();
            this.bufferedCommand = '';
            var stream = db.createReadStream();
